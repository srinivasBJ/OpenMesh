...
def workflow_node(entry: WorkflowEntry | dict[str, Any]) -> OpenMeshNode:
    ...
    return {
        "node_id": raw.get("node_id") or f"workflow:{_stable_id(str(raw.get('framework') or raw.get('source') or 'openmesh'))}:{_stable_id(str(raw.get('workflow') or raw.get('name') or 'workflow'))}",
        "node_type": "workflow",
...
def build_workflow_registry(records: Iterable[OpenMeshEventRecord]) -> list[dict[str, Any]]:
    ...
    for record in sorted(records, key=lambda item: item.timestamp):
        ...
        for node in (record.source_json, record.target_json):
            ...
            entry = entries.setdefault(
                node["node_id"],
                {
                    ...
                    "name": node.get("name"),
                    "framework": payload.get("framework") or metadata.get("framework") or node.get("runtime"),
            ...
            entry["framework"] = payload.get("framework", entry.get("framework")) or metadata.get("framework")
            entry["version"] = payload.get("version", entry.get("version")) or metadata.get("version")
            entry["source"] = payload.get("source", entry.get("source")) or metadata.get("source")
        ...
        if record.source_json and record.target_json:
            for node in (record.source_json, record.target_json):
                if node and node.get("node_type") == "workflow" and node["node_id"] in entries:
                    entries[node["node_id"]]["relationship_count"] += 1

    return sorted(entries.values(), key=lambda item: (str(item.get("workflow")).lower(), item["id"]))


async def get_workflow_registry(db: AsyncSession, limit: int = 5000) -> list[dict[str, Any]]:
...
def validate_workflow_entries(entries: Iterable[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    seen: dict[tuple[str, str], list[dict[str, Any]]] = {}
    missing = []
    malformed = []
    for entry in entries:
        missing_fields = [
            field for field in ("workflow", "framework", "source")
        ...
        seen.setdefault((str(entry.get("framework")), str(entry.get("workflow"))), []).append(entry)
    duplicates = [
...
def _stable_id(value: str) -> str:
    return "".join(character.lower() if character.isalnum() else "-" for character in value).strip("-") or "workflow"
